...
#endif
{
  tableBuilder_.cacheTable(std::vector<SampleFunc>({ sf::sin, sf::saw }));
  oscillator_ = new WaveTableOscillator(tableBuilder_.getTable());
  tableBuilder_.cacheTable(std::vector<SampleFunc>({ sf::sin, sf::sin }));
  modulator_ = new WaveTableOscillator(tableBuilder_.getTable());
}
...
void MyPluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
  ...
  oscillator_->setFrequency(500, sampleRate_);
  modulator_->setFrequency(1000, sampleRate_);
}
...
void MyPluginAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
  ...
  for (int i = 0; i < buffer.getNumSamples(); i++) {
    float modulation = modulator_->getNextSample(0.0f);
    float currentSample = oscillator_->getNextSample(modulation);
    channelData[i] = currentSample * level;
  }
  }
}
